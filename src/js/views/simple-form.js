...
export default (
  ...
  return class V extends Layout {
    ...
    serializeForm() {
      ...
      // labels
      var t0 = session.get( 'sp_lang', 'SP_IndexErrorMsg6') || Language.index.errorMsg6[this.lang];
      var t1 = session.get( 'sp_lang', 'SP_IndexErrorMsg7') || Language.index.errorMsg7[this.lang];
    ...
    changeInput( event ) {
      ...
      // labels
      var t0 = session.get( 'sp_lang', 'SP_Toast1') || Language.toast[1][this.lang];

      // validate input type
      if ( iClick == 1 || iClick == 4 || iClick == 5 || iClick == 7 || iClick == 11 ) {
        value = utils.toNumber( value );
        if ( isNaN(value) ) {
          return $.publish( 'toast', [2, $input.val() + t0] );
        }
      }
